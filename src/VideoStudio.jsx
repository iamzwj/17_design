...
export default function VideoStudio() {
  ...
  useEffect(() => {
    ...
    if (selectedModel.minimax && resolution === '1080p' && duration > 10) setDuration(10)
    if (locksAspectRatio) setAspectRatio('adaptive')
    if (locksDuration) setDuration(-1)
  }, [aspectRatio, aspectRatios, duration, locksAspectRatio, locksDuration, modes, referenceMode, resolution, selectedModel])
  ...
  const durationOptions = [-1, 4, 5, 6, 8, 10, 12, 15, 20, 30].filter((item) => item === -1 || (item <= selectedModel.maximum && !(selectedModel.minimax && resolution === '1080p' && item > 10)))
  ...
  return <section className="workspace video-waterfall-workspace">
    ...
    <div className="waterfall-composer-wrap">
      <div className="composer waterfall-composer glass-strong">
        <div className="video-waterfall-composer-title"><Icon name="video" size={16}/><b>视频生成</b><span>先选择素材如何参与生成</span></div>
